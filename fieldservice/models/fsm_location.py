...
class FSMLocation(models.Model):
    ...
    def create(self, vals_list):
        # ✅ FIX: when creating a child location, inherit partner from parent
        for vals in vals_list:
            parent_id = vals.get("fsm_parent_id")
            if parent_id and not vals.get("partner_id"):
    ...
    def _onchange_fsm_parent_id(self):
        ...
        # ✅ FIX UTAMA: partner ikut parent
        self.partner_id = self.fsm_parent_id.partner_id or False
    ...
    def comp_count(self, contact, equipment, loc):
        if equipment:
            ...
            if child_locs:
                for sub_loc in child_locs:
                    equip += sub_loc.comp_count(0, 1, sub_loc)
            return equip
        elif contact:
            for child in loc:
                child_locs = self.env["fsm.location"].search(
            ...
            if child_locs:
                for sub_loc in child_locs:
                    con += sub_loc.comp_count(1, 0, sub_loc)
            return con
        else:
            for child in loc:
                child_locs = self.env["fsm.location"].search(
            ...
            if child_locs:
                for sub_loc in child_locs:
                    subloc += sub_loc.comp_count(0, 0, sub_loc)
            return subloc
    ...
    def get_action_views(self, contact, equipment, loc):
        if equipment:
            ...
            if child_locs:
                for sub_loc in child_locs:
                    equip += sub_loc.get_action_views(0, 1, sub_loc)
            return equip
        elif contact:
            for child in loc:
                child_locs = self.env["fsm.location"].search(
            ...
            if child_locs:
                for sub_loc in child_locs:
                    con += sub_loc.get_action_views(1, 0, sub_loc)
            return con
        else:
            for child in loc:
                child_locs = self.env["fsm.location"].search(
            ...
            if child_locs:
                for sub_loc in child_locs:
                    subloc += sub_loc.get_action_views(0, 0, sub_loc)
            return subloc
